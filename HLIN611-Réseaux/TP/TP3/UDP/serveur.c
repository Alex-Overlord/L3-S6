...
int main(int argc, char *argv[]) {
  ...
  int rcv = recvfrom(ds, messagesRecus, sizeof(long int), 0, inet_ntoa(addrC.sin_addr), &lgAddrC);
    
  /* Traiter TOUTES les valeurs de retour (voir le cours ou la documentation) */
  if (rcv < 0) {
    perror("Serveur : [recvfrom] je suis sourd(e)\n");
    close(rcv);
    exit(1);
  } else if (rcv == 0) {
    printf("Serveur : [recvfrom] arrêt normal \n");
    break;
  }
